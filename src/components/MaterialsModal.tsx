...
import { X, FolderOpen, ArrowLeft, ChevronRight, FileText, Calendar, Layers } from 'lucide-react';
...
export const MaterialsModal: React.FC<MaterialsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs">
      <div
        className="bg-white rounded-3xl max-w-lg w-full p-5 sm:p-7 shadow-2xl border border-slate-100 flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-150"
        dir="ltr"
      >
        {/* Header */}
        ...
        <div className="py-4 overflow-y-auto flex-1 space-y-3">
          ...
          {/* LEVEL 3: Section Content (Placeholder ready for PDF files) */}
          {selectedBlock !== null && selectedSection !== null && (
            <div className="space-y-4">
              ...
              {/* PDF Container Ready Box */}
              <div className="py-8 px-4 bg-slate-50/80 rounded-2xl border-2 border-dashed border-slate-200 text-center space-y-3">
                <div className="w-12 h-12 rounded-2xl bg-white text-slate-600 flex items-center justify-center mx-auto shadow-2xs border border-slate-200">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-semibold mt-1" dir="rtl">
                    هذا القسم مهيأ ومخصص لرفع وإدراج ملفات الـ PDF وسنبدأ بإضافتها معاً خطوة بخطوة.
                  </p>
                </div>
              </div>
            </div>
